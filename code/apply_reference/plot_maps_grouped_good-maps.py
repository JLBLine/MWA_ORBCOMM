...
#def add_chi_sq(ax=None,pol_inds=None,alts=alts,gain=None,beam_map=None,beam_error=None,do_xlabel=False,do_ylabel=False):
...
def add_chi_sq(ax=None,pol_inds=None,alts=alts,gain=None,beam_map=None,beam_error=None,do_xlabel=False,do_ylabel=False,label=None,model_error=None):
    ...
    #if nanmin(chisq[where(chisq != -inf)]) < -20:
        #dax.set_ylim(-5,5)
    ...
    #weights = 1 / beam_error[pol_inds]
    #bad = where(weights == inf)
    #weights[bad] = 1.0
    #bad = where(weights == 'nan')
    #weights[bad] = 1.0

    #mask = where(isnan(chisq) == False)
    #poly_coeffs = polyfit(alts[mask], chisq[mask], 3)
    #poly3,poly2,poly1,poly0 = poly_coeffs

    #fit_line = poly3*alts**3 + poly2*alts**2 + poly1*alts + poly0
    #dax.plot(alts,fit_line,label=label)
...
fig_maps = plt.figure(figsize=(9*fraction,9.5*fraction))
...
def do_fit_plot(AUT=None,subplot=None,normed_beam=None):
    ...
    plt.figure(fig_maps.number)

    plot_healpix(data_map=corrected_map - gain,sub=(2,2,subplot),title=AUT,vmin=vmin,vmax=vmax,fig=fig_maps)
    ax = plt.gca()
    ax.text(0.07,0.92,labels[subplot-1],color='k',verticalalignment='center', horizontalalignment='center',transform=ax.transAxes,fontsize=20)

    ax = plt.gca()
    image = ax.get_images()[0]

    if subplot == 1:
        cax = fig_maps.add_axes([-0.005,0.53,0.015,0.4])
        cbar = fig_maps.colorbar(image, cax = cax,label='dB')
        cax.yaxis.set_ticks_position('left')
        cax.yaxis.set_label_position('left')
    elif subplot == 2:
        cax = fig_maps.add_axes([1.01,0.53,0.015,0.4])
        cbar = fig_maps.colorbar(image, cax = cax,label='dB')
    elif subplot == 3:
        cax = fig_maps.add_axes([-0.005,0.03,0.015,0.4])
        cbar = fig_maps.colorbar(image, cax = cax,label='dB')
        cax.yaxis.set_ticks_position('left')
        cax.yaxis.set_label_position('left')
    elif subplot == 4:
        cax = fig_maps.add_axes([1.01,0.03,0.015,0.4])
        cbar = fig_maps.colorbar(image, cax = cax,label='dB')
    ...
    #upper_errors = corrected_map[XX_inds] - gain + XX_null_errors
    #lower_errors = corrected_map[XX_inds] - gain - XX_null_errors
    #errors = corrected_map[XX_inds] - gain + ew_rf0
    #ax_XX.fill_between(alts, errors, corrected_map[XX_inds] - gain, facecolor='gray',alpha=0.5,zorder=0)
    ...
    #upper_errors = corrected_map[YY_inds] - gain + YY_null_errors
    #lower_errors = corrected_map[YY_inds] - gain - YY_null_errors
    #errors = corrected_map[YY_inds] - gain + ns_rf0
    #print ns_rf0

    #ax_YY.fill_between(alts, errors, corrected_map[YY_inds] - gain, facecolor='gray',alpha=0.4,zorder=0)
...
fig_maps.savefig('good_beammaps.pdf',bbox_inches='tight')
fig_maps.savefig('good_beammaps.png',bbox_inches='tight')
plt.figure(fig_maps.number)
plt.close()
...
#fig_diffs.savefig('bad_diffmaps.pdf',bbox_inches='tight')
#fig_diffs.savefig('bad_diffmaps.png',bbox_inches='tight')
#plt.figure(fig_diffs.number)
#plt.close()
